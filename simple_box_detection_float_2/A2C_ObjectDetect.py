...
def log(logstr):
    file_name = os.path.join('./save_model_3/', '220209.log')
        ...
        while not done:
            ...
            if agent.render:
                ...
                if done:
                    ...
                    torch.save(agent.model.state_dict(), './save_model_3/220209_episode_{}.pth'.format(e))
                if frame == max_frame:
                    score_avg = 0.9 * score_avg + 0.1 * score if score_avg != 0 else score
                    log("frame over 100 | episode : {} | score_avg : {} | loss : {} | sigma : {} | done : {}".format(e, score_avg, np.mean(loss_list), np.mean(sigma_list),done))
                    break
                if iou_frame == 300:
                    score_avg = 0.9 * score_avg + 0.1 * score if score_avg != 0 else score
                    log("frame over 300 | episode : {} | score_avg : {} | loss : {} | sigma : {} | done : {}".format(e, score_avg,
                                                                                                    np.mean(loss_list),
                                                                                                    np.mean(sigma_list),
                                                                                                    done))
                    break
