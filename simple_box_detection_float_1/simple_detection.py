...
class Env(tk.Tk):
    ...
    def _build_canvas(self):
        ...
        self.episode_text = canvas.create_text(40, 20, text="episode = {}".format(0), font=("나눔고딕코딩", 10),
                                               fill="blue")
        self.action_move = canvas.create_text(WIDTH*UNIT - (2*UNIT-10), HEIGHT*UNIT - 25,
                                                   text="action_move = {}".format(0),
                                                   font=("나눔고딕코딩", 10),
                                                   fill="green")
    ...
    def move_rect(self, action, idx):
        ...
        if s[0]+action[0] - 1 > 0 and s[2]+action[0] < WIDTH*UNIT - 1:
            self.canvas.delete(self.action_move)
            action_round = round(action[0], 2)
            self.action_move = self.canvas.create_text(WIDTH*UNIT - (2*UNIT-10), HEIGHT*UNIT - 25, text="action_move = {:.3f}".format(action_round),
                                                        font=("나눔고딕코딩", 10),
                                                        fill="green")
            x = action[0]
        # if idx == 1: # 우
