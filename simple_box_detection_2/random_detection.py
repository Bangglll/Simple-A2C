    ...
    def move_rect(self, action):
        ...
        if act == 2: # 위
            if s[1] > 0:
                y = -5
        if act == 3: # 아래
            if s[3] < HEIGHT*UNIT-1 :
                y = 5
        if act == 4: # widht_s
            if s[0] > 0 and s[2] < WIDTH*UNIT:
                x2 = s[2] - 5
                self.canvas.coords(self.r1, s[0], s[1], x2, s[3])
        if act == 5: # height
            if s[3] < HEIGHT*UNIT and s[1] > 0:
                y2 = s[3] - 5
                self.canvas.coords(self.r1, s[0], s[1], s[2], y2)
        if act == 6: # widht
            if s[0] > 0 and s[2] < WIDTH*UNIT :
                x2 = s[2] + 5
                self.canvas.coords(self.r1, s[0], s[1], x2, s[3])
        if act == 7: # height
            if s[3] < HEIGHT*UNIT and s[1] > 0 :
                y2 = s[3] + 5
                self.canvas.coords(self.r1, s[0], s[1], s[2], s[3])
        # if act == 8: # - x1
        #     if s[0] > 0 and s[2] < WIDTH*UNIT :
        #         x1 = s[0] - 5
        #         self.canvas.coords(self.r1, x1, s[1], s[2], s[3])
        # if act == 9: # - y1
        #     if s[3] < HEIGHT*UNIT and s[1] > 0 :
        #         y1 = s[1] - 5
        #         self.canvas.coords(self.r1, s[0], y1, s[2], s[3])
        # if act == 10: # + x1
        #     if s[0] > 0 and s[2] < WIDTH*UNIT :
        #         x1 = s[0] + 5
        #         self.canvas.coords(self.r1, x1, s[1], s[2], s[3])
